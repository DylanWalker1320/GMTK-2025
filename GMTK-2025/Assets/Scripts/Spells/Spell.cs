...
public abstract class Spell : MonoBehaviour
{
    ...
    {
        direction = FindAnyObjectByType<Reticle>().aimDirection;
        rb.GetComponent<Rigidbody2D>().linearVelocity = direction * speed; //normalized so that ball stays at a constant speed no matter how far mouse is from player
        transform.rotation = FindAnyObjectByType<Reticle>().transform.rotation;
    }
